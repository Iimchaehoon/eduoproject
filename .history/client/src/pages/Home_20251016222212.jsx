...
export default function Home() {
  ...
  const goSearch = (keyword) => {
    ...
    // 검색어가 있으면 ?q=만 넘긴다 (size 고정 X → Search.jsx가 필터된 개수만 표시)
        ...
        <div className="mt-7">
          ...
          <div className="flex flex-wrap gap-2.5 md:gap-3 items-center justify-center md:justify-start mt-5">
            {["인기 검색어", "데이터 사이언스", "머신러닝", "웹 개발", "AI"].map((t) => (
              <span
                key={t}
                className="px-3.5 h-9 inline-flex items-center rounded-full text-[12px] md:text-[13px] bg-white shadow-[0_6px_16px_rgba(16,24,40,0.06)] cursor-pointer"
                onClick={() => goSearch(t)}
              >
                {t}
              </span>
            ...
            <div className="flex gap-2 md:gap-3 ml-1 md:ml-2">
              <button
                className="px-4 h-9 rounded-full bg-[#EEF1FF] text-[#4450FF] text-[12px] md:text-[13px]"
                onClick={() => goSearch("")}
              >
                강좌 검색
              </button>
