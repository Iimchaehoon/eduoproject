...
/* 검색/카테고리 → 검색페이지로 이동 도우미 */
const goSearch = (nav, params) => {
  const usp = new URLSearchParams(params);
  nav({ pathname: "/search", search: `?${usp.toString()}` });
};
        ...
        <div className="grid grid-cols-5 lg:grid-cols-10 gap-4 mt-7">
          {categories.map((c) => (
            <div
              ...
              title={c.label}
              onClick={() => goSearch(nav, { category: c.label })}
            >
        ...
        <div className="mt-7">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  goSearch(nav, keyword ? { keyword } : {});
                }
              }}
            />
            <button
              className="ml-2 md:ml-3 px-5 h-9 md:h-10 rounded-full bg-[#E9ECFF] text-[#5B66FF] text-[13px] md:text-sm font-semibold cursor-pointer"
              onClick={() => {
          ...
          <div className="flex flex-wrap gap-2.5 md:gap-3 items-center justify-center md:justify-start mt-5">
            {["인기 검색어", "데이터 사이언스", "머신러닝", "웹 개발", "AI"].map((t) => (
              <button
                key={t}
                className="px-3.5 h-9 inline-flex items-center rounded-full text-[12px] md:text-[13px] bg-white shadow-[0_6px_16px_rgba(16,24,40,0.06)] hover:scale-105 transition-transform cursor-pointer"
                onClick={() => goSearch(nav, { keyword: t })}
              >
                {t}
              </button>
            ))}
            <div className="flex gap-2 md:gap-3 ml-1 md:ml-2">
              <button
                className="px-4 h-9 rounded-full bg-[#EEF1FF] text-[#4450FF] text-[12px] md:text-[13px] cursor-pointer"
                onClick={() => goSearch(nav, {})}
              >
                강좌 검색
              </button>
              <button
                className="px-4 h-9 rounded-full bg-[#E6E9EF] text-[#51607B] text-[12px] md:text-[13px] cursor-pointer"
                onClick={() => goSearch(nav, { sort: "최신순" })}
              >
                고급 검색
              </button>
            </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 mt-6">
          {aiRecs.map((r) => (
            <div
              ...
              onClick={() => nav(`/course/${r.slug}`)} // ✅ 상세페이지로 이동
              title={r.title}
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-3 flex items-center gap-1.5 text-[13px]">
                  <svg width="16" height="16" viewBox="0 0 20 20" fill="#F8B84A" className="shrink-0" aria-hidden="true">
