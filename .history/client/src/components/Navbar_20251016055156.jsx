...
export default function Navbar(){
  const Item = ({to, children}) => (
    <NavLink to={to} className={({isActive}) => `px-3 py-2 rounded-lg hover:bg-slate-100 ${isActive?'text-primary':'text-slate-700'}`}>{children}</NavLink>
  )
  return (
    <header className="bg-white/80 backdrop-blur sticky top-0 z-50 border-b border-skin-ring">
      <div className="max-w-6xl mx-auto px-5 h-16 flex items-center gap-6">
        <Link to="/" className="flex items-center gap-2">
          <img src="/img/main_logo.png" alt="EDUO" className="h-7 w-7 rounded-full" />
          <span className="font-semibold">EDUO</span>
        </Link>
